...
- 内置“加载示例报告”按钮，便于无模型环境下演示风险统计和事件时间线；示例明确标记为预置数据，非实时推理
...
使用 `http://127.0.0.1:8001/?demo=1` 可直接加载预置示例报告，用于展示事件时间线和风险统计。

## 示例报告

预置报告用于展示页面交互和审计结果结构，不代表模型实时推理结果。

![示例巡检报告](docs/demo-preview.png)
